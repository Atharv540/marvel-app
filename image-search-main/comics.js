...
auth.onAuthStateChanged(user=>{
    if (user){
        localStorage.setItem('comic-email', user["email"]);
        db.collection('accounts').where('id', '==', user["email"]).get().then(snapshot=>{
            snapshot.docs.forEach(doc=>{
                localStorage.setItem('comic-name', doc.data().name);
                localStorage.setItem('comic-logoUrl', doc.data().logoUrl);
                localStorage.setItem('comic-followers', doc.data().followers);
            })
        })
    ...
    displayImages = async (search_input_value) => {
        ...
        db.collection('comics').orderBy('rating', 'desc').get().then(snapshot=>{
            snapshot.docs.forEach(doc=>{
                ...
                if((doc.data().character.toLowerCase().includes(search_input_value.value) || search_input_value.value.includes(doc.data().character.toLowerCase()) || oneSameWord(search_input_value.value, doc.data().character.toLowerCase(), doc)===true) && displayed_images.includes(doc.data().imageUrl) === false){
                    ...
                    img.setAttribute('onclick', "doAsDirected('"+doc.data().imageUrl+"','"+doc.id+"','"+doc.data().character+"','"+doc.data().rating+"','"+doc.data().comic+"','"+doc.data().publisher+"')");
                    img.style.borderRadius = '15px';
        ...
        for(x=0; x<search_value_split.length; x++){
            for(y=0; y<db_value_split.length; y++){
                if(search_value_split[x] === db_value_split[y] && y!==db_value_split.length && db_value_split[y] !== 'man'){
                    let value = await db.collection('comics').doc(document.id).get()
                    image_present++
                    showImage(value);
                }
    ...
    submitToDatabase = async () => {
        ...
        let comic_input = document.querySelector('#comic-url').value
        if(image_input !== null && character_input!==null){
            if(user && image_input.split('').length>2 && character_input.split('').length>2){
                ...
                window.alert('You have to sign in, to add comics!');
            }
        }
    }
    ...
    showImage = async (value) => {
        if(lastUrl !== value.data().imageUrl && displayed_images.includes(value.data().imageUrl) === false){
            ...
            img.setAttribute('id', 'this_image');
            img.setAttribute('onclick', "doAsDirected('"+value.data().imageUrl+"','"+value.id+"','"+value.data().character+"','"+value.data().rating+"','"+value.data().comic+"','"+value.data().publisher+"')");
            img.style.borderRadius = '15px';
            img.style.marginLeft ='2%';
            img.style.height = '40%';
            img.style.marginTop = '50px';
    ...
    doAsDirected = async (url, id, character, rating, comic, publisher) => {
        let username = '';
        let logoUrl = '';
        let followers = '';
        ...
        if(preview_given === true){
            ...
            let previous_text4 = document.getElementById('text_preview4');
        ...
        p4.setAttribute('id', 'text_preview4');
        p4.textContent = followers+' followers';
        p4.style.fontFamily = 'Bad Script';
        ...
        button2.textContent = "Read Comic";
        button3.textContent = "Delete";
        button.setAttribute('onclick', 'rateImage("'+id+'")');
        button2.setAttribute('onclick', 'openItemWithUrl("'+id+'")')
        button3.setAttribute('onclick', 'deleteIfUser("'+id+'")');
        let img = document.createElement('img');
        ...
        await db.collection('comics').doc(id).get().then(snapshot=>{
    ...
    rateImage = async (id) => {
        let rating_input = window.prompt('What is your rating?');
        let user_rated = undefined
        await db.collection('comics').doc(id).get().then(snapshot=>{
            user_rated = snapshot.data().rated_users
        })
        if(user && rating_input<=5 && user_rated.includes(user['email'])===false && rating_input>=1){
            let current_raters = undefined
            let current_total_rating_number = undefined
            current_rated_users = undefined
            user_rated = undefined
            await db.collection('comics').doc(id).get().then(documents=>{
            ...
            if(rating_input<=5 && user_rated.includes(user['email'])===false && rating_input>=1){
                ...
                db.collection('comics').doc(id).update({
            ...
            window.alert('You have to sign in to rate comics!');
        } 
    ...
    openItemWithUrl = async (id) => {
        db.collection('comics').doc(id).get().then(snapshot=>{
            openItemInNewWindow(snapshot.data().comic)
        })
    }

    deleteIfUser = async (id) => {
        let publisher = undefined
        let modal = undefined;
        modal = document.querySelector('#modal-image-options');
        await db.collection('comics').doc(id).get().then(snapshot=>{
            publisher = snapshot.data().publisher
        })
        if(user && user["email"] === publisher){
            deleteItem(id);
            M.Modal.getInstance(modal).close();
        }
        else{
            window.alert("You can't delete somoene else's comic!")
        }
    }
