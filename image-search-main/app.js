...
auth.onAuthStateChanged(user=>{
    if (user){
        ...
        db.collection('accounts').where('id', '==', user["email"]).get().then(snapshot=>{
            snapshot.docs.forEach(doc=>{
                ...
                localStorage.setItem('followers', doc.data().followers);
            })
        })
    ...
    rateImage = async (id) => {
        ...
        if(user && rating_input<=5 && user_rated.includes(user['email'])===false && rating_input>=1){
            ...
            window.alert('You have to sign in to rate images!');
        } 
    ...
    openItemWithUrl = async (id) => {
        db.collection('images').doc(id).get().then(snapshot=>{
            openItemInNewWindow(snapshot.data().imageUrl)
        })
    }
    ...
    deleteIfUser = async (id) => {
        ...
        if(user && user["email"] === publisher){
            ...
            window.alert("You can't delete somoene else's image!")
        }
    }
